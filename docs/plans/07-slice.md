...
Behavior:
1. Load image.
2. Call `pixel.GuessGrid(img)` (from plan 03). If `Confidence < 0.5`, fall
   back to treating the whole image as a single row of unknown columns.
...
6. `sprite-gen spec` shows ten commands.
...
Extend internal/pixel with crop. Two new verbs: `slice grid`
...
- `slice auto` gutter detection reuses `GuessGrid` from plan 03. If that
  heuristic improves in plan 07, the improvement is automatically picked
  up here.
